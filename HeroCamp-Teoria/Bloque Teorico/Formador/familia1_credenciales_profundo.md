...
1. [Por qué esta familia abre la sesión](#1-por-qué-esta-familia-abre-la-sesión)
...
Arrancar la sesión con esta familia engancha a la audiencia con cifras que duelen, antes de entrar en categorías más técnicas como RLS o datos sensibles, que requieren más contexto para ser comprendidas.
...
La distinción más importante, y la que los PMs casi nunca manejan, es entre **claves de identificación** (pensadas para ser públicas, como una Google Maps API key embebida en frontend) y **claves de autenticación** (que dan acceso real y nunca deben salir del servidor). El problema es que esa distinción se está rompiendo: como veremos en el caso Gemini 2026, claves que Google consideraba "solo identificadores públicos" empezaron a autenticar contra Gemini silenciosamente, convirtiéndolas en credenciales de facto.

---

## 3. Mecanismo: por qué ocurre, especialmente con agentes IA

La cadena de eventos típica cuando un agente IA deja una credencial expuesta tiene siempre la misma forma. La describo aquí con detalle porque entender el mecanismo es lo que permite prevenirlo.
...
**Paso 5: Bots automatizados detectan la clave.** En repositorios públicos, bots especializados están monitorizando el stream de eventos de GitHub en tiempo real. La velocidad de detección es comentada en detalle en la sección 5.
...
Estos son los números que tienes que poder citar en clase sin dudar. Todos son verificables en las fuentes listadas al final.
...
Un dato que conviene tener a mano para el epic fail: en el caso concreto de ChatGPT, **más de 5.000 repositorios públicos de GitHub y 3.000 sitios web en producción** tenían claves de OpenAI hardcodeadas expuestas activamente, según Cyble Research.

---

## 5. Velocidad de explotación: el factor tiempo

Este es el dato que más impacta a la audiencia cuando se presenta bien. No hay margen para reaccionar; hay margen para prevenir.
...
Esta dimensión es la que más se olvida y la que más pesa cuando hay una crisis real. Un PM Senior debe pensar en círculos concéntricos.
...
Un dato concreto para ilustrar: en 2025, según IBM, **los ataques a través de cadena de suministro** (supply chain compromise) supusieron el 30% de los incidentes relacionados con modelos y aplicaciones de IA. Una credencial que expone un proveedor puede convertirse en la puerta de entrada a sus clientes.

---

## 8. Casos reales documentados

Aquí están los casos que puedes usar en clase, con datos verificados y referencias. Los ordeno por relevancia para la sesión.

### Caso 1 · Startup mexicana, $82.314 en 48 horas con Gemini (febrero 2026)

**Este es el epic fail principal recomendado.** Reciente, profesional, y con un giro narrativo perfecto.
...
Perfecto como caso ilustrativo del mecanismo completo desde commit hasta explotación.
...
El artículo original del incidente hace el análisis completo de fallos sistémicos: sin pre-commit hooks, IAM user con permisos excesivos, sin CloudWatch alarms en tiempo real, billing alert demasiado alto, sin límites de tasa en IAM.

**Mensaje que este caso permite transmitir:** el error individual (un commit despistado) es solo la chispa. Lo que hace que la factura llegue a 89K es la ausencia de capas defensivas. Una sola capa bien implementada (pre-commit hook, principio de mínimo privilegio, alerta en tiempo real) habría limitado el daño.
...
Útil para contexto de escala del problema específico con IA.
...
Esta sección es la que más diferencia tu sesión de una formación de seguridad clásica. Merece ser tratada con cuidado, sin ser alarmista pero sin minimizar.

**Lo que los datos dicen con claridad:** los commits asistidos por Claude Code en 2025 tuvieron una tasa de fuga de secretos del 3,2%, aproximadamente el doble del 1,5% de línea base del resto de GitHub. Dwayne McDaniel (GitGuardian) lo formuló bien en una entrevista reciente: "No estamos diciendo que la IA en sí sea mala. Lo que tenemos son los datos. Y los datos dicen que si co-firmabas tus commits con Claude Code durante 2025, tenías más del doble de probabilidades de commitear un secreto". Este dato debe usarse con honestidad: no es que Claude Code (o cualquier otro asistente) sea el culpable; es que la velocidad que habilitan cambia el régimen de fugas.
...
**Cómo enmarcar esto en clase sin asustar:** el agente IA no es el problema. El agente IA es un amplificador. Si tenías buenas prácticas de gestión de secretos, el agente te ayuda a ir más rápido sin problemas. Si no las tenías, el agente amplifica el riesgo. La solución no es dejar de usar el agente; es aprender las prácticas que permiten usarlo de forma segura.

---

## 10. Lo que el PM senior tiene que interiorizar

Cinco ideas clave que quiero que los alumnos se lleven de esta parte de la sesión. Cada una conecta con una pregunta que pueden hacerse ante cualquier situación futura.
...
**Casos específicos utilizables en clase:**
...
**Fin del documento de Familia 1.**

Próximos pasos sugeridos:
- Validar con Jaime que la profundidad y el tono encajan.
- Producir la versión ligera para alumnos (guía de referencia resumida).
- Pasar a la Familia 2 (Bases de datos y permisos) con el mismo nivel de profundidad.
