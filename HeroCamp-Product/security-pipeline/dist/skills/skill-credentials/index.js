...
exports.auditCredentials = auditCredentials;
...
async function auditCredentials(repoPath) {
...
function searchCredentialsInCode(repoPath) {
    const findings = [];
...
function checkEnvFile(repoPath) {
    const findings = [];
...
function searchCredentialsInGitHistory(repoPath) {
    const findings = [];
    try {
        // Obtener últimos 50 commits
        const commits = (0, child_process_1.execSync)(`cd "${repoPath}" && git log -50 --pretty=format:%H`, {
            encoding: 'utf-8'
        }).split('\n').filter(c => c);
        for (const commit of commits) {
            try {
                const diff = (0, child_process_1.execSync)(`cd "${repoPath}" && git show ${commit}`, {
                    encoding: 'utf-8',
...
function findSourceFiles(repoPath) {
