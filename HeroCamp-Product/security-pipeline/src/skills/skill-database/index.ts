...
export async function auditDatabase(repoPath: string): Promise<SkillResult> {
  ...
  try {
    ...
    if (credentials) {
      const activeCheckFindings = await verifyTablesWithAnonymousKey(
        repoPath,
        credentials
      );
...
function detectServiceRoleInClient(repoPath: string): Finding[] {
  const findings: Finding[] = [];
  ...
  for (const filePath of sourceFiles) {
    ...
    const serviceRoleMatches = Array.from(
      content.matchAll(/service_role[_\s]*key[:\s]*["']?([A-Za-z0-9_.\\-]{20,})/gi)
    );
...
function detectTablesWithoutRLS(repoPath: string): Finding[] {
  const findings: Finding[] = [];
  ...
  for (const filePath of sqlFiles) {
    ...
    const tableMatches = Array.from(
      content.matchAll(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/gi)
    );
    ...
    for (const match of tableMatches) {
      ...
      if (!tablesWithRLS.has(tableName.toLowerCase())) {
        ...
        const hasPendingComment = /TODO|PENDING|se activará en producción|will be enabled/i.test(
          beforeNextTable.substring(0, 500)
        );
...
function detectCosmeticRLS(repoPath: string): Finding[] {
  const findings: Finding[] = [];
  const sqlFiles = findSourceFiles(repoPath, ['.sql']);
  ...
  for (const filePath of sqlFiles) {
    ...
    const cosmeticMatches = Array.from(
      content.matchAll(/CREATE\s+POLICY\s+[a-zA-Z_][a-zA-Z0-9_]*\s+ON\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+.*?USING\s*\(\s*true\s*\)/gi)
    );
...
function extractSupabaseCredentials(repoPath: string): SupabaseCredentials | null {
  ...
  for (const filePath of sourceFiles) {
    ...
    const urlMatch = content.match(
      /SUPABASE_URL\s*[:=]\s*["']?(https:\/\/[a-zA-Z0-9-]+\.supabase\.co)["']?/i
    );

    // Buscar SUPABASE_ANON_KEY
    const anonKeyMatch = content.match(
      /SUPABASE_ANON_KEY\s*[:=]\s*["']?([A-Za-z0-9_\-.]+)["']?/i
    );
...
async function verifyTablesWithAnonymousKey(
  repoPath: string,
  ...
  for (const tableName of tables) {
    try {
      const response = await fetchWithTimeout(
        `${credentials.url}/rest/v1/${tableName}?select=*&limit=1`,
        {
          headers: {
